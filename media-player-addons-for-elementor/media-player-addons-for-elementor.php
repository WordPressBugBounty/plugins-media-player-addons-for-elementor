...
if ( function_exists( 'mpafe_fs' ) ) {
        mpafe_fs()->set_basename( true, __FILE__ );
} else {
	define('MPAFE_VERSION', isset($_SERVER['HTTP_HOST']) && 'localhost' === $_SERVER['HTTP_HOST'] ? time() : '1.1.6');
	define('MPAFE_DIR_URL', plugin_dir_url(__FILE__));
	define('MPAFE_DIR_PATH', plugin_dir_path(__FILE__));
		...
		function mpafe_fs() {
			...
			if ( ! isset( $mpafe_fs ) ) {
				...
				$mpafe_fs = fs_dynamic_init( array(
					...
					'menu'                => array(
						'slug'           => 'media-player-addons-for-elementor',
						'first-path'     => 'admin.php?page=media-player-addons-for-elementor#/welcome',
					),
				) );
			}
	...
	final class mpafe_main_element
	{
		...
		public function admin_notice_missing_main_plugin() {
			...
			echo '<p>' . esc_html__( '"Media Player Addons for Elementor" requires "Elementor" to be installed and activated.', 'media-player-addons-for-elementor' ) . '</p>';

			if ( ! $installed ) {
				echo '<p><button class="button button-primary" id="allembed-install-elementor">' . esc_html__( 'Install Elementor', 'media-player-addons-for-elementor' ) . '</button></p>';
			} elseif ( ! $active ) {
				echo '<p><button class="button button-primary" id="allembed-activate-elementor">' . esc_html__( 'Activate Elementor', 'media-player-addons-for-elementor' ) . '</button></p>';
			}
			echo '</div>';
		}
