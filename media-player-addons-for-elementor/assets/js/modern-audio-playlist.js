...
const baseTemplate = document.querySelector(".base-template");
const musics = JSON.parse(baseTemplate.dataset.musics || "[]");
const volume = parseFloat(baseTemplate.dataset.volume);
const random = baseTemplate.dataset.random === "true";

defaults.music = musics.length > 0 ? musics : [];
defaults.volume = !isNaN(volume) ? volume : 0.5;
defaults.random = random;
baseTemplate.removeAttribute("data-musics");
...
class MusicPlayer {
	constructor(config) {
		...
		this.playerBlock = document.querySelector(".modern-player");
	...
	playTrack(index) {
		if (this.music[index]) {
			...
			const artistName = this.music[index].artist;

			this.audio.src = this.music[index].url;
			this.artist.textContent = artistName || "Unknown Artist";
			this.title.textContent = artist || "Unknown Name hh";
	...
	setupPlaylist() {
		...
		for (let i = 0; i < this.music.length; i++) {
			const [title, subTitle] = this.music[i].title.split(" - ");
			const artistName = this.music[i].artist;
			...
			newMusicItem.innerHTML = `
                <span class="playlist__song">${title} - <span class="playlist__song-name">${artistName}</span></span>
